Type.registerNamespace('Core.EmbedJavaScriptJSOM');

Core.EmbedJavaScriptJSOM = function () {
    'use strict';
    // init: initializes the JavaScript Embed class, connects click events to buttons
    var init = function (sourceUrl, target, messageDiv) {
        messageElement = messageDiv;
        sourceFile = sourceUrl;
        targetFolder = target;
        // bind click events to the buttons
        $('#btnEmbed').click(provision);
        $('#btnRemove').click(unprovision);
        $('#' + messageElement).text('')
    },
    ...
    // un-provision: removes the custom action and the JavaScript file
    ...
    // unprovisionEx: internal method to remove the custom action and JavaScript file
    unprovisionEx = function () {
        ...
        while (enumerator.moveNext()) {
            ...
            if (action.get_description() == 'Core.EmbedJavaScriptJSOM' &&
        ...
        context.executeQueryAsync(
           function () {
               // ...and we're done
               $('#' + messageElement).text('Removal done!')
           },
           failure);
    },
    ...
    provisionScriptLink = function () {
        ...
        while (enumerator.moveNext()) {
            var action = enumerator.get_current();
            if (action.get_description() == 'SPAgent' &&
                action.get_location() == 'ScriptLink') {
                removeThese.push(action)
            }
        }
        ...
        newAction.set_description('SPAgent')
        ...
        context.executeQueryAsync(
            function () {
                // all is done, if we want to we can actually remove the app from the host web and still be happy about our JavaScript embedding
                $('#' + messageElement).text('Embed done!')
            },
            failure);
    },
...
$(document).ready(function () {
    // Usage: 
    //      sourceUrl: specify the JS file relative to the Pages folder (or the page where the script is executing)
    //      target: web relative folder/library where to store the file
    //      messageDiv: id of html element where messages are shown
    Core.EmbedJavaScriptJSOM.init('../Content/SPAgent.js', 'SiteAssets', 'message')
});